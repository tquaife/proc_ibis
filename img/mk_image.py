...
with open('../data_out/sif_o2a_linReg__foo.pickle', 'rb') as f:
    image = pickle.load(f)
sif_o2a_data = np.ma.masked_where(image < -10, image)
sif_o2a_data = sif_o2a_data[2000:,:]

with open('../data_out/sif_o2b_linReg__foo.pickle', 'rb') as f:
    image = pickle.load(f)
...
#plt.show()
plt.savefig("ibis_sif_linReg_o2a_o2b__foo.png",bbox_inches='tight', dpi=300)
